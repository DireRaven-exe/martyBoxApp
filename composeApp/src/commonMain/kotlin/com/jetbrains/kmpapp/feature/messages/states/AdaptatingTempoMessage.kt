...
class AdaptatingTempoMessage: StateMessage {
    override fun processMain(value: String, uiState: MutableStateFlow<MainUiState>): Boolean {
        return try {
            val cleanedValue = value.replace("\"", "").trim()
            val adaptatingTempo = cleanedValue.toBoolean()
            uiState.update { it.copy(adaptatingTempo = adaptatingTempo) }
            true
        } catch (e: Exception) {
            Napier.e(tag = "WebSocket", message = "Error parsing adaptatingTempo data: ${e.message}")
            false
        }
    }
