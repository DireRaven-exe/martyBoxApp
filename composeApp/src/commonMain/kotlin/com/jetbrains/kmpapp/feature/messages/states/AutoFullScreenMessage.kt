...
class AutoFullScreenMessage: StateMessage {
    override fun processMain(value: String, uiState: MutableStateFlow<MainUiState>): Boolean {
        return try {
            val cleanedValue = value.replace("\"", "").trim()
            val autoFullScreen = cleanedValue.toBoolean()
            uiState.update { it.copy(autoFullScreen = autoFullScreen) }
            true
        } catch (e: Exception) {
            Napier.e(tag = "WebSocket", message = "Error parsing autoFullScreen data: ${e.message}")
            false
        }
    }
