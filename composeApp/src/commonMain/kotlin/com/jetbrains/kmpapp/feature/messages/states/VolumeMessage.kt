...
class VolumeMessage: StateMessage {
    override fun process(value: String, uiState: MutableStateFlow<MainUiState>): Boolean {
        return try {
            val cleanedValue = value.replace("\"", "").trim()
            val volume = cleanedValue.toFloat()
            uiState.update { it.copy(volume = volume) }
            true
        } catch (e: Exception) {
            Napier.e(tag = "WebSocket", message = "Error parsing volume data: ${e.message}")
            false
        }
    }
}
